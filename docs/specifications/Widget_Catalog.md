...
The current cockpit implementation uses one session-local Application service. Scanner
selection publishes the Symbol with source `Scanner`; the Analysis workspace follows and
visibly renders `SELECTED` or `NO SELECTION`. Navigation does not change context, and no
context is persisted across application restarts.
...
- embedded read-only Price Chart in the Analysis workspace
- follows the shared Scanner or Watchlist instrument context
- explicit local JSON source selected only through `--price-history-json`
- immutable Application-owned UTC OHLCV bars
- exact decimal-string price validation and integer-volume validation
- strict oldest-first unique timestamp validation
- visible `NO SELECTION`, `LOADING`, `READY`, `NO DATA`, `UNAVAILABLE` and `ERROR` states
- visible source, timeframe, bar count and UTC period
- price candle and volume visualization
- manual reload for the active Symbol
- missing Symbols remain `NO DATA`; invalid payloads remain `ERROR`
- no inferred prices, fallback series, persistence, broker access or trading action

The current timeframe is defined by the configured source. Timeframe selection, stale-data
classification, indicators, drawing tools and multi-timeframe views remain future slices.
...
Selecting a visible result publishes session-local instrument context with source
`Scanner`.

---

## Instrument Analysis Workspace
...
Provide the safe workspace baseline for analysis of the active instrument.

Current implementation:

- dedicated read-only Analysis workspace
...
persistent analysis state remain future product slices. Missing analysis data is not
estimated, copied from another Symbol or generated by the UI.
